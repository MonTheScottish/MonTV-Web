...
export class MonTVRepository {
  ...
  private postProcessChannels(channels: Channel[]): Channel[] {
    const processed = channels.map((ch) => {
      // Generic mapper for VTV1-5, regional variants, and VTV Cần Thơ (VTV10) using VTVgo
      const vtvMatch = ch.id.match(/^vtv([1-5]|5-tay-nam-bo|5-tay-nguyen|-can-tho)(_|$)/);
      if (vtvMatch) {
        ...
        if (vtvgoId) {
          ...
          if (!foundWebview) {
            ...
            if (vtvgoId === "1") {
              ...
              const chanUpper = `VTV${vtvgoId === "6" ? "6" : vtvgoId === "10" ? "5_TN" : "5_TNB"}_HD`;
